...
ENV VIRTUAL_ENV /opt/sankey-streamlit-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/sankey-streamlit/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/sankey-streamlit/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/sankey_streamlit/ && touch src/sankey_streamlit/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/sankey-streamlit/
...
ENTRYPOINT ["/opt/sankey-streamlit-env/bin/poe"]
CMD ["app"]
